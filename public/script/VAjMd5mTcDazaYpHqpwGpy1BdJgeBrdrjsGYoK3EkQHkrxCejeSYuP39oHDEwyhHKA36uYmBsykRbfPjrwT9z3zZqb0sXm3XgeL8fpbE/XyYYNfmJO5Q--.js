          ...
          if (a === p.overlay) {
            ...
            !(function (e) {
              ...
              if (
                ...
                if (!c.comment.length)
                  return void w(jetpackCarouselStrings.no_comment_text, !1);
                if (
                  ...
                  if (!c.email.length || !c.email.match("@"))
                    return void w(jetpackCarouselStrings.no_comment_email, !1);
                  if (!c.author.length)
                    return void w(jetpackCarouselStrings.no_comment_author, !1);
                }
                  ...
                  (u.onreadystatechange = function () {
                    if (
                      ...
                      "approved" === e.comment_status
                        ? w(jetpackCarouselStrings.comment_approved, !0)
                        : "unapproved" === e.comment_status
                        ? w(jetpackCarouselStrings.comment_unapproved, !0)
                        : w(jetpackCarouselStrings.comment_post_error, !1),
                        H(),
                        q(a),
      ...
      if (
        ...
        if (a) {
          ...
          var i = o.parentElement,
            ...
            l = null;
          if (
            (n && n.classList.contains("wp-block-image")
              ? (l = i.getAttribute("href"))
              : i &&
                i.classList.contains("wp-block-image") &&
                i.querySelector(":scope > a") &&
                (l = i.querySelector(":scope > a").getAttribute("href")),
            l &&
              l.split("?")[0] !==
                o.getAttribute("data-orig-file").split("?")[0] &&
              l !== o.getAttribute("data-permalink"))
          )
            return;
  ...
  function init() {
    ...
    function getCommentId(link) {
      const commentId =
        ...
        link.getAttribute("href").split("like_comment=");
    ...
    function handleLinkAction(link, action, commentId, callback) {
      const nonce =
        link &&
        link.getAttribute("href") &&
        link.getAttribute("href").split("_wpnonce=")[1];
      ...
      renderLikesWithPagination(data, link) {
        swipeLibLoader().then(() => {
          ...
          let navContents = `
					...
					`;
          for (let i = 0; i < page_count; ++i) {
            navContents += `<em data-page="${i}" class="${
              i === 0 ? "on" : ""
            }">&bull;</em>`;
          }
          navContents += `
    ...
    function readCookie(c) {
      ...
      for (let i = 0; i < cookieStrings.length; i++) {
        ...
        if (cookieString.indexOf(nameEQ) === 0) {
          const chunk = cookieString.substring(
            nameEQ.length,
            cookieString.length
          );
            ...
            if (getServiceData()) {
              ...
              const commentLikeCheck = setInterval(() => {
                const iframe = document.querySelector(
                  "#wp-login-comment-nonce-iframe"
                );
                if (iframe) {
                  c = iframe.querySelector(
                    `#comment-like-${commentLikeId} .comment-like-link`
                  );
                }
    ...
    document.body.addEventListener("click", (e) => {
      ...
      handleLinkAction(target, action, commentId, (data) => {
        ...
        const countEl = document.querySelector(
          `#comment-like-count-${data.context}`
        );
        if (countEl) {
    ...
    document.body.addEventListener(
      ...
      function (e) {
        ...
        const star =
          container && container.querySelector("a.comment-like-link");
    ...
    document.body.addEventListener(
      ...
      (e) => {
        if (
          !e.target.matches(".comment") ||
          !e.target.querySelector("a.comment-like-link")
        ) {
          return;
        }
...
(function () {
  ...
  var findFirstLargeEnoughWidth = function (
    original_w,
    original_h,
    dest_w,
    dest_h
    ...
    for (var i = 0; i < prebuilt_widths.length; ++i) {
      if (
        prebuilt_widths[i] >= dest_w ||
        prebuilt_widths[i] * inverse_ratio >= dest_h
      ) {
  ...
  var generateImgSrc = function (srcItem, max) {
    ...
    var src =
      srcItem.getAttribute("src") ||
      srcItem.getAttribute("original") ||
      srcItem.getAttribute("data-original") ||
      srcItem.getAttribute("data-lazy-src");
